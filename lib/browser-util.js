...
const browseAsync = (browse, options) => {
  console.log('[ROON-API] browse.browse() called with:', JSON.stringify(options, null, 2));
  return promisify(browse.browse, browse)(options)
    .then(result => {
      console.log('[ROON-API] browse.browse() response:', JSON.stringify(result, null, 2));
      return result;
    })
    .catch(error => {
      console.log('[ROON-API] browse.browse() ERROR:', error.message || error);
      throw error;
    });
};

const loadAsync = (browse, options) => {
  console.log('[ROON-API] browse.load() called with:', JSON.stringify(options, null, 2));
  return promisify(browse.load, browse)(options)
    .then(result => {
      console.log('[ROON-API] browse.load() response:', JSON.stringify(result, null, 2));
      return result;
    })
    .catch(error => {
      console.log('[ROON-API] browse.load() ERROR:', error.message || error);
      throw error;
    });
};
